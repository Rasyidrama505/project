...
<head>
    ...
    <title>Data Rekap Murid SMA</title>
...
<div class="main-content p-4">
    ...
    <div class="dashboard-header d-flex justify-content-between align-items-center">
        <div class="d-flex align-items-center">
            ...
            <div>
                <h2 class="mb-0">Data Rekap Murid SMA Tahun 2022</h2>
    ...
    <div class="card mt-4">
        ...
        <div class="card-body">
            @php
                ...
                $total_x_l = $drm_sma->sum('siswa_tkt_x_l');
                $total_x_p = $drm_sma->sum('siswa_tkt_x_p');
                $total_xi_l = $drm_sma->sum('siswa_tkt_xi_l');
                $total_xi_p = $drm_sma->sum('siswa_tkt_xi_p');
                $total_xii_l = $drm_sma->sum('siswa_tkt_xii_l');
                $total_xii_p = $drm_sma->sum('siswa_tkt_xii_p');
                $total_xiii_l = $drm_sma->sum('siswa_tkt_xiii_l');
                $total_xiii_p = $drm_sma->sum('siswa_tkt_xiii_p');
                
                $grand_total_l = $total_x_l + $total_xi_l + $total_xii_l + $total_xiii_l;
                $grand_total_p = $total_x_p + $total_xi_p + $total_xii_p + $total_xiii_p;
                
                $total_kip_l = $drm_sma->sum('siswa_penerima_kip_l');
                $total_kip_p = $drm_sma->sum('siswa_penerima_kip_p');

                $total_dalam_kab = $drm_sma->sum('siswa_dari_dalam_kab');
                $total_luar_kab = $drm_sma->sum('siswa_dari_luar_kab');
            @endphp
                ...
                <table class="table table-bordered text-center">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{ number_format($total_kip_l) }}</td>
                            <td>{{ number_format($total_kip_p) }}</td>
                            <td><strong>{{ number_format($total_kip_l + $total_kip_p) }}</strong></td>
                        </tr>
                        <tr>
    ...
    <div class="table-responsive mt-4">
        <table class="table table-striped table-hover table-bordered">
            ...
            <tbody class="text-center align-middle">
                ...
                @empty
                <tr>
                    <td colspan="17" class="text-center">Tidak ada data rekap murid SMA</td>
                </tr>
                @endforelse
            </tbody>
        </table>
