...
<head>
    ...
    <title>Data Identitas Sekolah SMK</title>
...
<div class="main-content p-4">
    ...
    <div class="dashboard-header d-flex justify-content-between align-items-center">
        <div class="d-flex align-items-center">
            ...
            <div>
                <h2 class="mb-0">Data Identitas Sekolah SMK Tahun 2021</h2>
            ...
            <div class="row">
                <div class="col-md-3">
                    <table class="table table-sm table-bordered text-center">
                        ...
                        <tbody>
                            <tr><td class="text-start">Negeri</td><td>{{ $dis_smk->filter(fn($item) => strtolower(trim($item->status)) === 'negeri')->count() }}</td></tr>
                            <tr><td class="text-start">Swasta</td><td>{{ $dis_smk->filter(fn($item) => strtolower(trim($item->status)) === 'swasta')->count() }}</td></tr>
                            <tr class="table-light">
                                <td class="text-start"><strong>Total</strong></td>
                                <td><strong>{{ $dis_smk->count() }}</strong></td>
                            </tr>
                        </tbody>
                    </table>
                </div>
                ...
                <div class="col-md-3">
                    @php
                        $kabupatenCounts = $dis_smk->groupBy(fn($item) => trim($item->kabupaten_kota));
                    @endphp
                    <table class="table table-sm table-bordered text-center">
                        <thead class="table-light">
                ...
                <div class="col-md-3">
                    @php
                        $akreditasiCounts = $dis_smk->groupBy('akreditasi');
                        $akreditasiOptions = $dis_smk->pluck('akreditasi')->filter()->unique()->sort();
                    @endphp
                    <table class="table table-sm table-bordered text-center">
                        <thead class="table-light">
                ...
                <div class="col-md-3">
                    <table class="table table-sm table-bordered text-center">
                        <thead class="table-light">
                            <tr><th colspan="2">Jumlah Rombongan Belajar</th></tr>
                        </thead>
                        <tbody>
                            <tr><td class="text-start">Tingkat X</td><td>{{ $dis_smk->sum('jml_rombel_tkt_x') }}</td></tr>
                            <tr><td class="text-start">Tingkat XI</td><td>{{ $dis_smk->sum('jml_rombel_tkt_xi') }}</td></tr>
                            <tr><td class="text-start">Tingkat XII</td><td>{{ $dis_smk->sum('jml_rombel_tkt_xii') }}</td></tr>
                            <tr><td class="text-start">Tingkat XIII</td><td>{{ $dis_smk->sum('jml_rombel_tkt_xiii') }}</td></tr>
                            <tr class="table-light">
                                <td class="text-start"><strong>Total Rombel</strong></td>
                                <td><strong>{{ $dis_smk->sum('jml_rombel_tkt_x') + $dis_smk->sum('jml_rombel_tkt_xi') + $dis_smk->sum('jml_rombel_tkt_xii') + $dis_smk->sum('jml_rombel_tkt_xiii') }}</strong></td>
                            </tr>
                        </tbody>
                    </table>
                </div>
    ...
    <div class="table-responsive mt-4">
        <table class="table table-striped table-hover table-bordered">
            ...
            <tbody class="text-center align-middle">
                ...
                @empty
                <tr>
                    <td colspan="15" class="text-center">Tidak ada data identitas sekolah SMK</td>
                </tr>
                @endforelse
            </tbody>
        </table>
