...
<head>
    ...
    <title>Data Rekap Murid SLB</title>
...
<div class="main-content p-4">
    ...
    <div class="dashboard-header d-flex justify-content-between align-items-center">
        <div class="d-flex align-items-center">
            ...
            <div>
                <h2 class="mb-0">Data Rekap Murid SLB Tahun 2024</h2>
    ...
    <div class="card mt-4">
        ...
        <div class="card-body">
            @php
                ...
                $total_x_l = $drm_slb->sum('siswa_tkt_x_l');
                $total_x_p = $drm_slb->sum('siswa_tkt_x_p');
                $total_xi_l = $drm_slb->sum('siswa_tkt_xi_l');
                $total_xi_p = $drm_slb->sum('siswa_tkt_xi_p');
                $total_xii_l = $drm_slb->sum('siswa_tkt_xii_l');
                $total_xii_p = $drm_slb->sum('siswa_tkt_xii_p');
                $total_xiii_l = $drm_slb->sum('siswa_tkt_xiii_l');
                $total_xiii_p = $drm_slb->sum('siswa_tkt_xiii_p');

                $grand_total_l = $total_x_l + $total_xi_l + $total_xii_l + $total_xiii_l;
                $grand_total_p = $total_x_p + $total_xi_p + $total_xii_p + $total_xiii_p;

                $total_kip = $drm_slb->sum('siswa_penerima_kip');
                $total_dalam_diy = $drm_slb->sum('siswa_dari_dalam_kab');
                $total_luar_diy = $drm_slb->sum('siswa_dari_luar_kab');
            @endphp
            <div class="table-responsive">
                <table class="table table-bordered text-center">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td class="text-start"><strong>Domisili Dalam DIY</strong></td>
                            <td colspan="3"><strong>{{ number_format($total_dalam_diy) }}</strong></td>
                        </tr>
                        <tr>
                            <td class="text-start"><strong>Domisili Luar DIY</strong></td>
                            <td colspan="3"><strong>{{ number_format($total_luar_diy) }}</strong></td>
                        </tr>
                    </tbody>
                </table>
            </div>
    ...
    <div class="table-responsive mt-4">
        <table class="table table-striped table-hover table-bordered">
            ...
            <tbody class="text-center align-middle">
                ...
                @empty
                <tr>
                    <td colspan="16" class="text-center">Tidak ada data rekap murid SLB</td>
                </tr>
                @endforelse
            </tbody>
        </table>
