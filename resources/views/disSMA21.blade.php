...
<head>
    ...
    <title>Data Identitas Sekolah SMA</title>
...
<div class="main-content p-4">
    ...
    <div class="dashboard-header d-flex justify-content-between align-items-center">
        <div class="d-flex align-items-center">
            ...
            <div>
                <h2 class="mb-0">Data Identitas Sekolah SMA Tahun 2021</h2>
            ...
            <div class="row">
                <div class="col-md-3">
                    <table class="table table-sm table-bordered text-center">
                        ...
                        <tbody>
                            <tr><td class="text-start">Negeri</td><td>{{ $sma->filter(fn($item) => strtolower(trim($item->status)) === 'negeri')->count() }}</td></tr>
                            <tr><td class="text-start">Swasta</td><td>{{ $sma->filter(fn($item) => strtolower(trim($item->status)) === 'swasta')->count() }}</td></tr>
                            <tr class="table-light">
                                <td class="text-start"><strong>Total</strong></td>
                                <td><strong>{{ $sma->count() }}</strong></td>
                            </tr>
                        </tbody>
                    </table>
                </div>
                ...
                <div class="col-md-3">
                    @php
                        $kabupatenCounts = $sma->groupBy(fn($item) => trim($item->kabupaten_kota));
                    @endphp
                    <table class="table table-sm table-bordered text-center">
                        <thead class="table-light">
                ...
                <div class="col-md-3">
                    @php
                        $akreditasiCounts = $sma->groupBy('akreditasi');
                        $akreditasiOptions = $sma->pluck('akreditasi')->filter()->unique()->sort();
                    @endphp
                    <table class="table table-sm table-bordered text-center">
                        <thead class="table-light">
                ...
                <div class="col-md-3">
                    <table class="table table-sm table-bordered text-center">
                        ...
                        <tbody>
                            <tr><td class="text-start">Tingkat X</td><td>{{ $sma->sum('jml_rombel_tkt_x') }}</td></tr>
                            <tr><td class="text-start">Tingkat XI</td><td>{{ $sma->sum('jml_rombel_tkt_xi') }}</td></tr>
                            <tr><td class="text-start">Tingkat XII</td><td>{{ $sma->sum('jml_rombel_tkt_xii') }}</td></tr>
                            <tr><td class="text-start">Tingkat XIII</td><td>{{ $sma->sum('jml_rombel_tkt_xiii') }}</td></tr>
                            <tr class="table-light">
                                <td class="text-start"><strong>Total Rombel</strong></td>
                                <td><strong>{{ $sma->sum('jml_rombel_tkt_x') + $sma->sum('jml_rombel_tkt_xi') + $sma->sum('jml_rombel_tkt_xii') + $sma->sum('jml_rombel_tkt_xiii') }}</strong></td>
                            </tr>
                        </tbody>
                    </table>
                </div>
    ...
    <div class="table-responsive mt-4">
        <table class="table table-striped table-hover table-bordered">
            ...
            <tbody class="text-center align-middle">
                ...
                @empty
                <tr>
                    <td colspan="15" class="text-center">Tidak ada data identitas sekolah SMA</td>
                </tr>
                @endforelse
            </tbody>
        </table>
